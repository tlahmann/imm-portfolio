<?php
/**
 * Custom post type Supervisor class
 * The class Supervisor provides a new post type for the WP backbone. It stores
 * supervisors teaching at ulm university.
 *
 * @package WordPress
 * @subpackage imm
 * @since imm 0.0.26
...
class Supervisor
{
    ...
    {
        add_action('init', array( $this, 'register_supervisors_post_type' ));
        add_filter('wp_insert_post_data', array( $this, 'check_supervisor_featured_image' ), 10, 2);
        add_action('admin_notices', array( $this, 'show_supervisor_featured_image_warning_message' ));
    }
     ...
     * @return void
     */
    public function register_supervisors_post_type(): void
    {
        ...
        $args = array(
            ...
            */
            ...
            'labels'              => array(
                'name'               => _x('Betreuer', 'post type general name'),
                'singular_name'      => _x('Betreuer', 'post type singular name'),
                'add_new'            => __('Neuen Betreuer anlegen'),
                'add_new_item'       => __('Neuen Betreuer anlegen'),
                'edit_item'          => __('Betreuer-Daten bearbeiten'),
                'new_item'           => __('Neuen Betreuer'),
                'all_items'          => __('Alle Betreuer'),
                'view_item'          => __('Betreuer ansehen'),
                'search_items'       => __('Betreuer durchsuchen'),
                'not_found'          => __('Keine Betreuer gefunden'),
                'not_found_in_trash' => __('Keine Betreuer im Papierkorb gefunden'),
                'parent_item_colon'  => '',
                'menu_name'          => 'Betreuer'
            ),
        );
    ...
    /**
     * Show (echo) a warning message if one post of type \Supervisor has no post thumbnail attached
     *
     * @return void
     */
    public function show_supervisor_featured_image_warning_message(): void
    {
        $post = get_post();
        if (!isset($post) || $post->post_type !== 'supervisor') {
            return;
        }
